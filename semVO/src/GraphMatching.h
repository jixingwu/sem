...
public:
    ...
    // match without vote tensor
    void matchTwoTopoMetricTradition(GraphT* tm2);
    ...
    // calculate prob of all nodes under this observation
    ...
    //
    Eigen::Vector3d getBestTpriorvisited();
...
void GraphMatching<GraphT, NodeT>::matchTwoTopoMetric(GraphT* tm2)
{
    ...
    visited_graph = tm2;
    clear();
//    cout << "graph prior "<< prior_graph->size()
//    << " nodes, graph visited "<< visited_graph->size() << " nodes." <<endl;
...
template <typename GraphT, typename NodeT>
void GraphMatching<GraphT, NodeT>::matchTwoTopoMetricTradition(GraphT* tm2)
{
    cout<<"[GM]match with traditional method"<<endl;
    visited_graph = tm2;
    clear();
...
//    ofstream ost(graph_path+"Gloc_tra_simmat.txt");
...
//            cout<<"node(pri): "<<i_1<<", node(visi): "<<i_2<<endl;
            score = matchTwoNodeDirectly(prior_graph->getNode(i_1), visited_graph->getNode(i_2),
                                 q_prior_visited, t_prior_visited, retmatch);
//            cout<<", q: ("<<q_prior_visited.w()
//               <<", "<<q_prior_visited.x()<<", "<<q_prior_visited.y()<<", "<<q_prior_visited.z()
//               <<"), t: "<<t_prior_visited<<endl;
...
void GraphMatching<GraphT, NodeT>::calculateValueOfVoteTensor(Eigen::Vector3d t_prior_visited)
{
    ...
    for(int i_pri=0; i_pri<prior_graph->size(); ++i_pri)
    {
        ...
        /*for(int i_vec=0; i_vec<nodezero_vec.size(); ++i_vec)
        {
            ...
            float center_dist = (p_prior - p_visited_tran).norm();
            if(center_dist < permitted_center_dist){
        ...
        // all nodes method 2-- check value of similarity then rotation err
        /*for(int i_visi=0; i_visi<visited_graph->size(); ++i_visi)
        {
            if(tmpSim_mat(i_pri, i_visi) < vote_threshold) continue;
        }*/
...
//        cout<<sim_mat<<endl;
//        cout<<"row:"<<maxRow<<", col:"<<maxCol<<", maxvalue:"<<maxvalue<<endl;
...
/*template <typename GraphT, typename NodeT>
double GraphMatching<GraphT, NodeT>::calProbWithSimmatMatch(Eigen::MatrixXd sim_mat, Eigen::VectorXd retmatch)
{
    double retprob = 1;
    for(int i=0; i<sim_mat.rows(); ++i)
    {
        if(retmatch(i) < 0){
            retprob /= sim_mat.rows();
        }
        else{
            retprob *= sim_mat(i, retmatch(i));
        }
    }
    return retprob;
}*/
...
template <typename GraphT, typename NodeT>
Eigen::Vector3d GraphMatching<GraphT, NodeT>::getBestTpriorvisited()
{
    if(!is_matched){
    ...
    return turntot(index_bestmatch(0), index_bestmatch(1), index_bestmatch(2));
}
...
/*
template <typename GraphT, typename NodeT>
int GraphMatching<GraphT, NodeT>::getPuzzleZoneID(int node_id, int choose_graph)
{
    int id_prior = node_id;
    if(choose_graph == 1){
        id_prior = getIDPriorFromIDVisited(node_id);
    ...
    if(id_prior < 0 || id_prior >= prior_graph->size()){
        cout<<"[GM]cannot get puzzle zone with node id "<<id_prior<<endl;
        return -1;
    }
    return node_to_puzzle_zone[id_prior];
}*/
...
//            for(int gi=0; gi<split_nodes[si].size(); ++gi)
//            {
//                node_to_puzzle_zone[split_nodes[si][gi]].push_back(puzzle_zone_vec.size()-1);
//            }
//            cout<<endl;
        }
    }
